...
import com.genius.note.MemoActivity;
import com.genius.note.R;
import com.genius.sync.Evernote;
import com.genius.utils.DateHelper;

import java.util.HashMap;
import java.util.Iterator;
import java.util.Set;

public class MemosAdapter extends CursorAdapter implements OnClickListener, OnLongClickListener {
    ...
    public MemosAdapter(Context context, Cursor c, int flags, ItemLongPressedLisener itemLongPressedLisener, onItemSelectLisener selectLisener) {
        super(context, c, flags);
        mLayoutInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        mRobotoThin = Typeface.createFromAsset(context.getAssets(), "fonts/Roboto-Thin.ttf");
        mContext.getContentResolver().registerContentObserver(MemoProvider.MEMO_URI, false, new UpdateObserver(mUpdateHandler));
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if (cursor != null && view != null && _id != 0) {
            ...
            if (null != searchKey && searchKey.length() > 0) {
                String string = Html.fromHtml(memo.getContent()).toString();
                if (!string.contains(searchKey)) {
                    view.setVisibility(View.GONE);
                    hoverView.setVisibility(View.GONE);
                } else {
                    SpannableString spannableString = new SpannableString(string);
                    spannableString.setSpan(new ForegroundColorSpan(Color.RED), string.indexOf(searchKey), string.indexOf(searchKey) + searchKey.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        ...
        if (cursor.getInt(cursor.getColumnIndex("_id")) == 0) {
            View firstView = mLayoutInflater.inflate(R.layout.memo_add, parent, false);
            ...
            View commonView = mLayoutInflater.inflate(R.layout.memo_item, parent, false);
    ...
    public void deleteSelectedMemos() {
        if (mCheckedItems == null || mCheckedItems.size() == 0) {
            ...
            while (iterator.hasNext()) {
                mContext.getContentResolver().delete(ContentUris.withAppendedId(MemoProvider.MEMO_URI, iterator.next()), null, null);
            }
            mCheckedItems.clear();
            mOnItemSelectLisener.onCancelSelect();
